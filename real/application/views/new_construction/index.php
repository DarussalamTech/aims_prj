...
<base target="_parent" />
<title>Candace Courter - New Construction</title>
...
<div class="ad_container"> 
	<div id="contentRightWrapper" style="float:left;">
        ...
        <div class="advancedSearch">
          <h1 class="shadowHeader">New Construction Search</h1>
           <form class="standardForm" method="post" action="#" id="newConstructionForm">
                <div class="clearfix" id="advanced-search-form-wrapper">
                  ...
                  <input type="button" onClick="search_new_construction();" style="width:83px;" class="btn-search customBlueButton" value="">
...
function Parent_FloatMenu(){
	...
	var newPosition=scrollAmount - 800;
	if(newPosition < 220 || newPosition > 2800){
		return false;
	}
...
$(document).ready(function(data){
	...
	$("#newConstructionForm").submit(function(e) {
		e.preventDefault();
		search_new_construction();
	});	
	...
	$("#newConstructionForm :input").change(function() {
		get_count();
	});
...
function get_count(){
	$.ajax({
		type: 'POST',
		url: '<?=$base?>new_construction/get_results_count/',
		data: $( "#newConstructionForm" ).serialize(),
		dataType: 'json',
		beforeSend: function(){
			//loader here
...
function search_new_construction(){	
	var offset = $('#search_offset').val();	
	$.ajax({
		type: 'POST',
		url: '<?=$base?>new_construction/search_results/18/'+offset,
		data: $( "#newConstructionForm" ).serialize(),
		dataType: 'json',
		beforeSend: function(){
			$.blockUI({
