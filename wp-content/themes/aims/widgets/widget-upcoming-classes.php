...
class upcoming_classes_widget extends WP_Widget 
{
    ...
    function widget($args, $instance) 
	{
		...
		$query = "SELECT TIME_FORMAT(t1.start, '%H.%i') AS start, TIME_FORMAT(t1.end, '%H.%i') AS end, t2.post_title, t2.post_name FROM wp_" . $blog_id . "_class_hours AS t1 
		...
		if(count($categories))
			$query .= "
				...
				WHERE $wpdb->terms.name IN ('" . join("','", (array)$categories) . "')
		...
		if($class_hours_count):
				...
				for($i=0; $i<$class_hours_count; $i++)
				{
					if($mode=="12h")
					{
						$class_hours[$i]->start = date("h.i a", strtotime($class_hours[$i]->start));
						$class_hours[$i]->end = date("h.i a", strtotime($class_hours[$i]->end));
					}
		...
		$output .= '[/items_list]';
			...
			$output .= '<p>' . __('No upcoming classes for today', $themename) . '</p>';
		endif;
		$output .= '</div>
		</li>';
	...
	{	
		...
		<p>
			<label for="<?php echo $this->get_field_id('mode'); ?>"><?php _e('Mode', $themename); ?></label>
			<select name="<?php echo $this->get_field_name('mode'); ?>">
				<option value="24h"<?php echo ($mode=="24h" ? " selected='selected'" : ""); ?>><?php _e('24h', $themename); ?></option>
				<option value="12h"<?php echo ($mode=="12h" ? " selected='selected'" : ""); ?>><?php _e('12h', $themename); ?></option>
			</select>
		</p>
		<p>
