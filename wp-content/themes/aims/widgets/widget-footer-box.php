...
class footer_box_widget extends WP_Widget 
{
    ...
    function footer_box_widget() 
	{
        ...
        parent::WP_Widget('gymbase_footer_box', __('Footer Box', $themename), $widget_options, $control_options);
    }
	...
	{	
		...
		<p>
			<label for="<?php echo $this->get_field_id('url'); ?>"><?php _e('Url', $themename); ?></label>
		...
		<p>
			...
			<select id="<?php echo $this->get_field_id('color'); ?>" name="<?php echo $this->get_field_name('color'); ?>">
				<option <?php echo ($color=='super_light_green' ? ' selected="selected"':'');?> value='super_light_green'><?php _e("super_light_green", $themename); ?></option>
