...
<a href="javascript:;" onClick="show_advanced_form();" style="background-color:#0A57A3;padding:10px;color:#fff;font-weight:bold">Search again</a>
</div>
...
if(!empty($properties) && $properties != 'noresult'){ ?>
    ...
    <div id="tab4" class="tabContents" style="min-height:1415px;">
     ...
     //   print_r(objectToArray($properties));
        ...
        //$this->session->set_userdata('res_sess',$properties);
        foreach($array_property as $property_details){
            $last_class = '';
            $address = '';
          //  print_r($property_details ["ml_num"]);
          // echo $property_details ["ml_num"];
        ...
        ?>
             <td><div class="searchResult"> <a class="propPhotoWrapper" title="View Full Property Details For <?php echo $address; ?>"  onclick="recently_viewed('<?php echo $property_details["ml_num"]; ?>');" href="<?php echo $home_site_url.'/property-details/?details='.$property_details["ml_num"]; ?>">
             <img src="<?php echo $property_details["first_image"]; ?>" />

              </a>
              <div class="propDetails">
                  <h3> <a title="View Full Property Details For  <?php echo $address;?>" onclick="recently_viewed('<?php echo $property_details["ml_num"]; ?>');" href="<?php echo $home_site_url.'/property-details/?details='.$property_details["ml_num"]; ?>">
                <?php 
                    if(!empty($property_details["address"])){
         ...
         echo   $property_details["city_name"].', '.$property_details["rets_state"].' '.$property_details["zip_code"];
          ...
          <?php
            ...
            //print_r($this->session->userdata('res_sess'));
}else {
...
function recently_viewed(ml_num){

	var previous_ml_nums = $.cookie('ml_nums_cookie');
	//console.log(previous_ml_nums);
	var new_value = previous_ml_nums+' '+ml_num;
	//console.log(new_value);
	$.cookie('ml_nums_cookie', new_value, { expires: 7, path: '/', domain: 'bonniestrickland.com' });
	
	//var all_cook = $.cookie('ml_nums_cookie');
