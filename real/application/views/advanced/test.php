...
<base target="_parent">
	...
	<title><?php echo !empty($candace_title)?$candace_title.' | ':''; ?>Bonnie Strickland</title>
...
<meta content="<?php echo !empty($candace_title)?$candace_title:''; ?> - Bonnie Strickland" property="og:title">
<meta content="<?php echo !empty($candace_description)?$candace_description:'Bonnie Strickland - South Tampa Real Estate | Real Estate News Blog | New Construction - Dolphin Homes. MLS Search - New Construction Search | Integrity'; ?>" property="og:description">
...
<h1 class="the_title">Property Details</h1>
</div>	
...
<div id="left-panel" style="float: left; width: 210px;border-right:1px solid #F0ECED;">
  <div class="widget widget_text" id="text-12" style="margin-top:15px;border-bottom:1px solid #F0ECED;padding-bottom:15px;margin-bottom:20px;">
    <div class="textwidget">
	  <a class="nyroModal custom-button medium" style="background-color: #a78432 !important; margin-bottom:5%;" href="<?php echo $base.'schedule/index/'.$property_details["ml_num"];?>">Schedule a showing</a>
      <br />
      ...
      <div id="make_prop_fav">
      		<?php if(empty($is_fav)){ ?>
      		<a href="<?php echo $base.'property_details/add_to_favorite/'.$property_details["ml_num"];?>" class="nyroModal_fav">
            <img style="float:left" src="<?php echo base_url(); ?>bootstrap/img/favorite.png"></a>
            <span style="float:left;margin-left:15px;margin-top:10px;">
            	<a class="nyroModal_fav" href="<?php echo $base.'property_details/add_to_favorite/'.$property_details["ml_num"];?>">Favorites</a>
            </span>
            <?php }else{ ?>
	  ...
	  <div style="min-height:70px;">
      	<?php if(empty($logged_in)){ ?>
            ...
            <span style="font-size:small;margin-left:15px;">
            	<a href="#">Login</a> or <a href="#">Register</a> to use</span>
        <?php }else{ ?>
            <img src="<?php echo base_url(); ?>bootstrap/img/notes.png" style="float:left;padding-top:3px;">
            Property Notes<br>
            ...
            <span style="display:none;" id="add_notes">
                <span id="notes_error" style="display:none"></span>
      ...
      <h3 style="line-height:20px;">The Strickland Group</h3>
      201 2nd Ave N, St Petersburg, FL 33701<br>
      727.432.6982 - Phone<br>
      <a href="mailto:Bonnie@bonniestrickland.com">Bonnie@bonniestrickland.com</a><br>
    </div>
  </div>
      ...
      <table style="margin-bottom:0px;">
		  <?php
            if(!empty($recently_viewed) && is_array($recently_viewed)){
                foreach ($recently_viewed as $recent_property) {
                    $address = '';
                    if(!empty($recent_property["49"])){
                         $address .= $recent_property["49"].', ';
                    }	
                    $address .= $recent_property["2302"].', '.$recent_property["2304"].' '.$recent_property["46"];
                ?>
                    <tr>
                        <td>
                            <div class="searchResult"> 
                                <h5> 
                                    <a title="View Full Property Details For  <?php echo $address; ?>" href="<?php echo "http://" . $_SERVER['HTTP_HOST'].'/property-details/?details='.$recent_property["ml_num"]; ?>">
                                        <?php 
                                             if(!empty($recent_property["49"])){
                                                echo $recent_property["49"].', ';
                                            }
                                            echo $recent_property["2302"].', '.$recent_property["2304"].' '.$recent_property["46"];
                                         ?>
                                    </a> 
                                </h5>
                                <div style="float:left;width:60px;">
	                                <a class="propPhotoWrapper" title="View Full Property Details For <?php echo $address; ?>" href="<?php echo "http://" . $_SERVER['HTTP_HOST'].'/property-details/'.$recent_property["ml_num"]; ?>">
									<img src="<?php echo $property_details['first_image']; ?>" style="width:60px;height:60px;" >
        	                        </a>
                                </div>
                                <div class="propDetails" style="float:right;">
                                  ...
                                  <p class="priceBlock"> <span class="price">
                                    <?php
										if(!empty($recent_property["176"])){
											if($recent_property["176"] < 10){
												echo '$' . number_format($recent_property["176"], 0, ".", ",").',000,000';
											}elseif($recent_property["176"] > 10 && $recent_property["176"] < 1000){
												echo '$' . number_format($recent_property["176"], 0, ".", ",").',000';
											}else{
												echo '$' . number_format($recent_property["176"], 0, ".", ","); 
											}
										}else{
                                  ...
                                  <?php if((!empty($recent_property["32"])) || (!empty($recent_property["2294"])) || (!empty($recent_property["2346"])) ){ ?>
    
                                    <?php 
                                        if(!empty($recent_property["32"])){
                                            echo $recent_property["32"].' Bed | '; 
                                        }
                                        if(!empty($recent_property["2294"])){
                                            echo $recent_property["2294"].' Bath'; 
                                        }
                                        if(!empty($recent_property["2346"])){
                                            echo '</br>'.number_format($recent_property["2346"], 0, ".", ",").' Sq Ft'; 
                                        }
                                    ?>
                                  </p>
                                  <?php } ?>
...
<div class="clear"></div>
<!-- AddThis Button END -->
  <input type="hidden" id="current_url" value="<?php echo $base; ?>property-details/<?php echo $property_details["ml_num"] ?>" />
			...
			$('a.nyroModal_remove').nyroModal({
				...
				callbacks: {
					afterClose: function() {
						...
						rem_html +=	'<span style="float:left;margin-left:15px;margin-top:10px;">'
						rem_html +=		'<a class="nyroModal_fav" href="<?php echo base_url(); ?>property_details/add_to_favorite/<?php echo $property_details["175"]; ?>">Favorites</a>'
						rem_html +=	'</span>'
						$("#make_prop_fav").html(rem_html);	
