    ...
    function index($hash = null) {
		...
		if (isset($hash)) {
            ...
            if ($this->account_model->check_forgot_password_hash($hash) > 0) {
				...
				$this->email->subject('New Password for '.WEBSITE_NAME);
				$this->email->from('nasir.lahore@gmail.com', WEBSITE_NAME);
		...
		exit();
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$message = $this->load->view('emails/forgot_password_request_email_view', $this->data, TRUE);			
				$config['charset'] = "utf-8";
				$config['mailtype'] = "html";
				...
				$this->email->set_newline("\r\n");
				$this->email->subject(WEBSITE_NAME.' Change Password Request');
				$this->email->from('nasir.lahore@gmail.com', WEBSITE_NAME);
