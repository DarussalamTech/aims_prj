...
function theme_inner_classes_custom_box_main($post)
{
	...
	<table>
		...
		<tr>
			<td>
				<label for="color">' . __('Timetable box background color', $themename) . ':</label>
			</td>
			<td>
				<input class="regular-text color" type="text" id="color" name="color" value="' . esc_attr(get_post_meta($post->ID, $themename . "_color", true)) . '" />
			</td>
		</tr>
		<tr>
			<td>
				<label for="text_color">' . __('Timetable box text color', $themename) . ':</label>
			</td>
			<td>
				<input class="regular-text color" type="text" id="text_color" name="text_color" value="' . esc_attr(get_post_meta($post->ID, $themename . "_text_color", true)) . '" />
			</td>
		</tr>
	</table>';
}
...
function classes_edit_columns($columns)
{
	...
	$columns = array(  
		...
		"title" => _x('Title', 'post type singular name', $themename),
		"classes_category" => __('Categories', $themename),
		...
		while(have_posts()): the_post();
			...
			$output .= '<div class="text">
					...
					<div class="item_footer clearfix">
						<a class="more icon_small_arrow margin_right_white" href="' . $classes_url . '/#' . urldecode($post->post_name) . '" title="' . __("Details", $themename) . '">' . __("Details", $themename) . '</a>
						<a class="more icon_small_arrow margin_right_white" href="' . $timetable_url . '/#' . urldecode($post->post_name) . '" title="' . __("Timetable", $themename) . '">' . __("Timetable", $themename) . '</a>
					</div>
				</div>
			</li>';
		endwhile;
		...
		while(have_posts()): the_post();
			...
			$output .= '<li>
				...
				<div class="clearfix tabs">
					<ul>
						<li>
							<a href="#' . urldecode($post->post_name) . '-about" title="' . __("About", $themename) . '">' . __("About", $themename) . '</a>
						</li>';
						if(count($trainers)):
						$output .= '
						<li>
							<a href="#' . urldecode($post->post_name) . '-trainers" title="' . __("Trainers", $themename) . '">' . __("Trainers", $themename) . '</a>
						</li>';
						endif;
						$output .= '
						<li>
							<a href="' . $timetable_url . '/#' . urldecode($post->post_name) . '" title="' . __("Timetable", $themename) . '">' . __("Timetable", $themename) . '</a>
						</li>
					</ul>
				...
				$output .= '
					<div id="' . urldecode($post->post_name) . '-trainers">
						<ul>';
						$i = 0;
